...
import '../../../lib/data/models/dto/enhanced_dto_mapper.dart';
import '../../../lib/data/models/dto/mapping_utils.dart';
import '../../../lib/data/models/dto/mapping_error_reporter.dart';
import '../../../lib/data/models/interfaces/dto_mapper.dart';
  ...
  group('Enhanced DTO Mapper Tests', () {
    ...
    group('Error Handling and Reporting', () {
      test('should report mapping errors with detailed context', () {
        ...
        final entity = mapper.toEntity(dto);
        expect(entity.id,
            equals(0)); // Should be 0 because 'invalid_id' can't be parsed
      ...
      test('should handle validation errors properly', () {
        ...
        expect(() => mapper.toEntityValidated(dto),
      ...
      test('should provide error statistics', () {
        ...
        for (int i = 0; i < 5; i++) {
          try {
            final dto =
                TestDto({'name': ''}); // Empty name should fail validation
        ...
        expect(stats.totalErrors,
            greaterThanOrEqualTo(0)); // May have errors from processing
      });
    ...
    group('Data Transformation', () {
      test('should apply nested transformers correctly', () {
        final dto = TestDto({
          ...
          'created_at':
              '2023-01-01T00:00:00Z', // String that should be converted to DateTime
...
class TestMapper extends EnhancedDtoMapper<TestEntity, TestDto>
    with CommonFieldTransformers {
  @override
  Map<String, NestedTransformer> get nestedTransformers => {
        ...
        'categories': ListTransformer<Map<String, dynamic>>(
  ...
  TestEntity createEntityFromData(Map<String, dynamic> data) {
    ...
    entity.name = MappingUtils.getNestedValue<String>(data, 'name') ??
        MappingUtils.getNestedValue<String>(data, 'title') ??
        ''; // Handle migrated field
    entity.userEmail = MappingUtils.getNestedValue<String>(data, 'user.email');
    entity.avatar =
        MappingUtils.getNestedValue<String>(data, 'user.profile.avatar');
    entity.theme = MappingUtils.getNestedValue<String>(
            data, 'user.profile.settings.theme') ??
        MappingUtils.getNestedValue<String>(data, 'theme') ??
        'system';
    entity.notifications = MappingUtils.getNestedValue<bool>(
            data, 'user.profile.settings.notifications') ??
        false;
    entity.isActive =
        MappingUtils.getNestedValue<bool>(data, 'is_active') ?? false;
    entity.createdAt =
        MappingUtils.getNestedValue<DateTime>(data, 'created_at');
    entity.updatedAt =
        MappingUtils.getNestedValue<DateTime>(data, 'updated_at');
    entity.originalText =
        MappingUtils.getNestedValue<String>(data, 'original_text');
    entity.tags = MappingUtils.getNestedValue<List<String>>(data, 'tags') ?? [];
    entity.categories = MappingUtils.getNestedValue<List<Map<String, dynamic>>>(
            data, 'metadata.categories') ??
        [];
    entity.status = MappingUtils.getNestedValue<TestStatus>(data, 'status') ??
        TestStatus.inactive;
    return entity;
  }
  ...
  @override
  ValidationResult validateCustomFields(Map<String, dynamic> data,
      {required bool isDto}) {
    final errors = <String>[];
