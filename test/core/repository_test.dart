...
void main() {
  group('Repository Pattern Tests', () {
    ...
    test('Cache strategies work correctly', () {
      ...
      expect(
          timeBased.isValid('test', now.subtract(const Duration(minutes: 15))),
          isTrue);

      // Invalid cache (expired)
      expect(timeBased.isValid('test', now.subtract(const Duration(hours: 1))),
          isFalse);
    ...
    test('Repository-specific cache strategies are configured correctly', () {
      ...
      final settingsStrategy =
          RepositoryCacheStrategies.settingsStrategy<UserSettings>();
      expect(settingsStrategy, isA<WriteThrough<UserSettings>>());

      // Test inbox note strategy
      final noteStrategy =
          RepositoryCacheStrategies.inboxNoteStrategy<dynamic>();
      expect(noteStrategy, isA<TimeBased<dynamic>>());

      // Test audio upload strategy
      final audioStrategy =
          RepositoryCacheStrategies.audioUploadStrategy<AudioUploadResult>();
      expect(audioStrategy, isA<TimeBased<AudioUploadResult>>());

      // Test transcription strategy
      final transcriptionStrategy = RepositoryCacheStrategies
          .transcriptionStrategy<TranscriptionResult>();
