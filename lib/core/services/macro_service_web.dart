...
class MacroService {
  ...
  Future<void> init() async {
    ...
    print(
        "MacroService (Web): Starting initialization with in-memory storage...");
    _isInitialized = true;
  ...
  Future<void> seedDefaultMacros() async {
    ...
    print(
        "MacroService (Web): ✅ Default macros seeded in memory (${_inMemoryMacros.length} total)");
  }
  ...
  Future<void> updateMacro(int id, String trigger, String content,
    ...
    if (index != -1) {
      ...
      if (aiInstruction != null)
        _inMemoryMacros[index].aiInstruction = aiInstruction;
