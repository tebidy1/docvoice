...
class _EditorScreenState extends State<EditorScreen> {
  ...
  void initState() {
    ...
    if (widget.oneShotAudioPath != null) {
      ...
      if (widget.draftNote != null) {
        ...
        _generatedOutputs =
            List<GeneratedOutput>.from(widget.draftNote!.generatedOutputs);
      }
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        final deptId =
            DepartmentService().value ?? prefs.getString('specialty');
      ...
      _backgroundTranscriptionFuture = () async {
        final transcript = await GeminiTranscriptionHelper()
            .transcribeFromPath(widget.oneShotAudioPath!);
    ...
    if (_generatedOutputs.isNotEmpty) {
      ...
      _activeTabIndex =
          _generatedOutputs.length; // Select the latest generated map
      _finalController.text = _generatedOutputs.last.content ?? "";
    } else if (widget.draftNote != null &&
      ...
      _finalController.text =
          _sourceController.text; // Default to transcript if nothing generated
    }
    ...
    if (audioPath == null || audioPath.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('⚡ No audio file available for One-Shot AI.'),
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (response.statusCode != 200) {
          throw Exception(
              'Failed to fetch audio blob (${response.statusCode})');
        }
  ...
  Future<void> _saveDraftUpdate() async {
    ...
    try {
      ...
      await inboxService.updateNote(
        _currentNoteId!,
        rawText: _sourceController.text.isNotEmpty
            ? _sourceController.text
            : 'لا يوجد نص اصلي عند اختيار هذا النموذج', // REQUIRED by backend
        formattedText: _generatedOutputs.isNotEmpty
            ? _generatedOutputs.last.content
            : '', // Legacy fallback
  ...
  Future<void> _applyMacroWithAI(Macro macro) async {
    ...
    if (_backgroundTranscriptionFuture != null) {
      ...
      if (_sourceController.text.isEmpty ||
          _sourceController.text.startsWith('❌')) {
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    _isOneShotMode
                        ? "⚡ Choose Template (One-Shot)"
                        : "Choose Template",
                    style: GoogleFonts.inter(
                      fontSize: 16,
                      fontWeight: FontWeight.w600,
                      color:
                          _isOneShotMode ? Colors.amber : colorScheme.onSurface,
                    ),
                  ),
