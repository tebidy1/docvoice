...
class SherpaLocalService {
  ...
  static const double _noiseGateThreshold =
      0.005; // Lowered: 0.01 was cutting soft consonants
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      if (!await isModelReady()) {
        _lastError =
            'Model not downloaded. Please download the offline model in Settings.';
        print('❌ $_lastError');
        return;
      }
      ...
      final vadConfig = sherpa.VadModelConfig(
        sileroVad: sherpa.SileroVadModelConfig(
          ...
          threshold: 0.5,
          minSilenceDuration:
              0.8, // Was 0.5: raised to avoid splitting mid-sentence
          minSpeechDuration:
              1.5, // Was 0.25: raised so Whisper gets full phrases, not fragments
          windowSize: 512,
        ),
        sampleRate: 16000,
        numThreads: 1,
  ...
  Future<String> transcribeAudioFile(String audioFilePath) async {
    ...
    try {
      ...
      final List<Float32List> mergedSegments =
          _mergeSegments(rawSegments, waveData.sampleRate);
  ...
  List<Float32List> _mergeSegments(List<Float32List> segments, int sampleRate) {
    ...
    final int maxChunkSamples =
        30 * sampleRate; // 30 seconds max per Whisper window
