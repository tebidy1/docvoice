...
class _MacroManagerDialogState extends State<MacroManagerDialog> {
  ...
  Future<void> _loadMacros() async {
    ...
    try {
      ...
      final deptNameEn =
          deptId != null ? MedicalDepartments.getById(deptId)?.nameEn : null;

      final filtered = all.where((m) {
        final cats = m.category.split(',').map((e) => e.trim()).toList();
        if (cats.isEmpty ||
            cats.contains('General') ||
            cats.contains('General Practice')) return true;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          ...
          action: SnackBarAction(
              label: 'Retry', textColor: Colors.white, onPressed: _loadMacros),
        ));
      }
    }
  }
  ...
  List<Macro> _applyCategory(List<Macro> pool) {
    switch (_selectedCategory) {
      case 'Favorites':
        return pool.where((m) => m.isFavorite).toList();
      case 'Most Used':
  ...
  List<String> get _departmentCategories {
    ...
    for (final m in _allMacros) {
      for (final c in m.category.split(',').map((e) => e.trim())) {
        if (c.isNotEmpty && c != 'General' && c != 'General Practice')
          cats.add(c);
      }
    }
  ...
  Future<void> _addOrEditMacro({Macro? macro}) async {
    ...
    final deptItems = MedicalDepartments.all
            ...
            actions: [
              ...
              FilledButton(
                ...
                onPressed: isSaving
                    ? null
                    : () async {
                        if (triggerCtrl.text.isEmpty ||
                            contentCtrl.text.isEmpty) {
                          ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content:
                                      Text('Please fill Trigger and Content'),
                          ...
                          await Future.any([
                            macro == null
                                ? _macroService.addMacro(
                                    triggerCtrl.text, contentCtrl.text,
                                    isAiMacro: isAiMacro,
                                    aiInstruction: aiInstrCtrl.text,
                                    category: selectedCategory)
                                : _macroService.updateMacro(macro.id,
                                    triggerCtrl.text, contentCtrl.text,
                                    isAiMacro: isAiMacro,
                                    aiInstruction: aiInstrCtrl.text,
                                    category: selectedCategory),
  ...
  Widget _dlgField({
    ...
    return TextField(
      ...
      style: const TextStyle(color: Colors.white, fontSize: 13),
      cursorColor: _kBlue,
      decoration: _dlgDecoration(label: label, icon: icon).copyWith(
          hintText: hint,
      ...
      child: Row(children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Manage your templates',
              style: TextStyle(
                  color: theme.iconColor.withValues(alpha: 0.45),
                  fontSize: 11)),
        ]),
        const Spacer(),
        // + New Macro
        ...
        IconButton(
          icon: Icon(Icons.settings_outlined,
              color: theme.iconColor.withValues(alpha: 0.5), size: 20),
          onPressed: () => showDialog(
              context: context, builder: (_) => const MacroSettingsDialog()),
          tooltip: 'Settings',
        ),
        IconButton(
        ...
        Padding(
          ...
          child: Row(children: [
            Icon(Icons.folder_outlined,
                color: theme.iconColor.withValues(alpha: 0.6), size: 16),
            const SizedBox(width: 8),
            Text('Categories',
                style: TextStyle(
                    color: theme.iconColor,
              ...
              if (userDept != null) ...[
                ...
                Padding(
                  ...
                  child: Text('MY DEPARTMENT',
                      style: TextStyle(
                          color: theme.iconColor.withValues(alpha: 0.4),
                          fontSize: 10,
                          letterSpacing: 1.0)),
                ),
  ...
  Widget _sidebarItem(
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color:
              isSelected ? _kBlue.withValues(alpha: 0.12) : Colors.transparent,
          borderRadius: BorderRadius.circular(8),
        ),
        child: Row(children: [
          ...
          Expanded(
              child: Text(name,
                  style: TextStyle(
                    color: isSelected
                        ? _kBlue
                        : theme.iconColor.withValues(alpha: 0.7),
                    fontSize: 13,
                    fontWeight:
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        child: Row(children: [
          Icon(dept.icon,
              size: 15,
              color: isSelected ? _kBlue : dept.color.withValues(alpha: 0.8)),
          const SizedBox(width: 8),
          Expanded(
        ...
        Padding(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              hintStyle:
                  TextStyle(color: theme.iconColor.withValues(alpha: 0.4)),
              prefixIcon: Icon(Icons.search,
                  color: theme.iconColor.withValues(alpha: 0.4), size: 20),
  ...
  Widget _emptyState(ThemePreset theme) {
    return Center(
      ...
      Icon(Icons.search_off,
          color: theme.iconColor.withValues(alpha: 0.15), size: 56),
      const SizedBox(height: 12),
      Text('No macros found',
          style: TextStyle(
              color: theme.iconColor.withValues(alpha: 0.4), fontSize: 14)),
    ]));
  }
          ...
          Tooltip(
            ...
            child: IconButton(
              icon: Icon(Icons.copy_outlined,
                  color: theme.iconColor.withValues(alpha: 0.45), size: 18),
              onPressed: () async {
  ...
  Widget _categoryBadge(String category) {
    ...
    return Container(
      ...
      child: Text(first,
          style: const TextStyle(
              color: _kBlue, fontSize: 10, fontWeight: FontWeight.w600)),
    );
  }
