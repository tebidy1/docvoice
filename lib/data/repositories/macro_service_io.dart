...
import '../../core/entities/macro_io.dart';
...
class MacroService {
  ...
  Future<String> getMacrosAsJson() async {
    try {
      final macros = await getAllMacros();
      final List<Map<String, dynamic>> jsonList = macros.map<Map<String, dynamic>>((m) => {
        'id': m.id,
        'trigger': m.trigger,
        'content': m.content,
        'category': m.category,
  ...
  /// Alias used by settings screens — clears all macros then re-seeds defaults.
