...
class LocalMacroRepository extends AbstractLocalRepository<Macro>
    implements MacroRepository {
  final MacroDtoMapper _mapper = MacroDtoMapper();
  final StreamController<List<Macro>> _watchController =
  ...
  Future<List<String>> getCategories() async {
    try {
      final allMacros = await fetchAll();
      final categories =
          allMacros.map((macro) => macro.category).toSet().toList();
      categories.sort();
      return categories;
    } catch (e) {
