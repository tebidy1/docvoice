...
import '../../core/repositories/audio_service.dart';
import '../../core/repositories/base_service.dart';
...
class AudioServiceImpl extends AudioService with ServiceLifecycle {
  ...
  Future<AudioValidationResult> validateAudioFile(File audioFile) async {
    ...
    try {
      ...
      if (!_supportedFormats.contains(fileExtension)) {
        errors.add(
            'Unsupported audio format: $fileExtension. Supported formats: ${_supportedFormats.join(', ')}');
      }

      // Validate file size
      if (fileSize > _maxFileSizeBytes) {
        final maxSizeMB =
            (_maxFileSizeBytes / (1024 * 1024)).toStringAsFixed(1);
        final currentSizeMB = (fileSize / (1024 * 1024)).toStringAsFixed(1);
        errors.add(
            'File size too large: ${currentSizeMB}MB. Maximum allowed: ${maxSizeMB}MB');
      }

      if (fileSize < _minFileSizeBytes) {
        errors.add(
            'File size too small: ${fileSize}B. Minimum required: ${_minFileSizeBytes}B');
      }
      ...
      try {
        ...
        if (bytes.isEmpty) {
          ...
          if (!_isValidAudioFileSignature(bytes, fileExtension)) {
            errors.add(
                'File does not appear to be a valid $fileExtension audio file');
          }
        }
      } catch (e) {
  ...
  Future<AudioUploadResult> uploadAudio(File audioFile) async {
    ...
    try {
      ...
      final noteData = {
        'title':
            'Audio Recording - ${DateTime.now().toString().substring(0, 19)}',
  ...
  void _startTranscriptionPolling(
      String audioId, StreamController<TranscriptionStatus> controller) {
