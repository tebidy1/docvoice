...
class OciRequestSigner {
  ...
  Map<String, String> signRequestUri({
    ...
    if (body != null && body.isNotEmpty) {
      ...
      headersToSign
          .addAll(['x-content-sha256', 'content-length', 'content-type']);
    }

    // 3. Build the signing string
    final requestTarget = '${method.toLowerCase()} ${_requestTargetPath(uri)}';
  ...
  String _httpDate() {
    // Adjust for server time difference (server appears to be 9 hours behind)
    final now = DateTime.now().toUtc().subtract(const Duration(hours: 9));
