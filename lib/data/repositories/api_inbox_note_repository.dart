import 'dart:async';
import 'package:soutnote/core/repositories/abstract_repository.dart';
import 'package:soutnote/core/repositories/inbox_note_repository.dart';
import 'package:soutnote/data/models/dto/inbox_note_dto.dart';
...
class ApiInboxNoteRepository extends AbstractApiRepository<InboxNote>
  ...
  Future<List<InboxNote>> getPending() async {
    try {
      final response = await _ApiClient.get('$endpoint/pending');
  ...
  Future<List<InboxNote>> getArchived() async {
    try {
      final response = await _ApiClient.get('$endpoint/archived');
  ...
  Future<void> updateStatus(String id, NoteStatus status) async {
    try {
      final statusString = status.toString().split('.').last;
      final response = await _ApiClient.patch('$endpoint/$id/status', body: {
        'status': statusString,
      });
  ...
  Future<void> archive(String id) async {
    try {
      final response = await _ApiClient.patch('$endpoint/$id/archive');
  ...
  Future<List<InboxNote>> getNotesWithAudio() async {
    try {
      final response = await _ApiClient.get('$endpoint/with-audio');
