...
class MappingErrorReporter {
  static final MappingErrorReporter _instance =
  ...
  MappingErrorStatistics getStatistics() {
    ...
    for (final error in recent24h) {
      ...
      errorsByMapper[error.mapperType] =
          (errorsByMapper[error.mapperType] ?? 0) + 1;

      if (error.fieldName != null) {
        errorsByField[error.fieldName!] =
            (errorsByField[error.fieldName!] ?? 0) + 1;
      }
    }
  ...
  Map<String, dynamic> getDetailedReport(MappingErrorReport report) {
    ...
    if (report.originalData != null) {
      context['original_data_analysis'] =
          _analyzeOriginalData(report.originalData);
    }
  ...
  List<MappingErrorReport> getErrorsForMapper(String mapperType,
      {int limit = 10}) {
  ...
  List<MappingErrorReport> getErrorsForField(String fieldName,
      {int limit = 10}) {
    return _errorHistory
  ...
  List<RecurringErrorPattern> findRecurringPatterns() {
    ...
    for (final error in _errorHistory) {
      final key =
          '${error.mapperType}:${error.fieldName}:${error.error.runtimeType}';
  ...
  Map<String, dynamic> _analyzeOriginalData(dynamic data) {
    ...
    if (data is List) {
      return {
        ...
        'length': data.length,
        'item_types':
            data.map((item) => item.runtimeType.toString()).toSet().toList(),
  ...
  int _calculateNestingDepth(Map<String, dynamic> data,
      [int currentDepth = 0]) {
