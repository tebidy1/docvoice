...
class UserDtoMapper extends EnhancedDtoMapper<User, UserDto>
  ...
  Map<String, dynamic> extractDataFromEntity(User entity) => {
        ...
        if (entity.lastSeen != null)
          'last_seen': entity.lastSeen!.toIso8601String(),
  ...
  User createEntityFromData(Map<String, dynamic> data) {
    return User(
      ...
      profileImageUrl:
          MappingUtils.getNestedValue<String>(data, 'profile_image_url'),
      createdAt: MappingUtils.getNestedValue<DateTime>(data, 'created_at') ??
          DateTime.now(),
      updatedAt: MappingUtils.getNestedValue<DateTime>(data, 'updated_at') ??
          DateTime.now(),
    );
  }
  ...
  ValidationResult validateCustomFields(Map<String, dynamic> data,
    ...
    if (role != null && !_isValidRole(role)) {
      errors
          .add('Invalid role. Must be one of: admin, company_manager, member');
    }
