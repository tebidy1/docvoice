...
import 'package:soutnote/core/services/settings_service_interface.dart';
...
class UserSettingsDtoMapper
    extends EnhancedDtoMapper<UserSettings, UserSettingsDto>
  ...
  UserSettings createEntityFromData(Map<String, dynamic> data) {
    ...
    final settingsData =
        MappingUtils.getNestedValue<Map<String, dynamic>>(data, 'settings') ??
            data;

    return UserSettings(
      settings: Map<String, dynamic>.from(settingsData),
      lastModified:
          MappingUtils.getNestedValue<DateTime>(data, 'last_modified') ??
              DateTime.now(),
  ...
  ValidationResult validateCustomFields(Map<String, dynamic> data,
    ...
    final language =
        MappingUtils.getNestedValue<String>(settingsData, 'language');
    if (language != null && !_isValidLanguage(language)) {
      errors.add('Invalid language code format');
    }

    final audioQuality =
        MappingUtils.getNestedValue<String>(settingsData, 'audio_quality');
    if (audioQuality != null && !_isValidAudioQuality(audioQuality)) {
      errors.add(
          'Invalid audio_quality value. Must be one of: low, medium, high');
    }
