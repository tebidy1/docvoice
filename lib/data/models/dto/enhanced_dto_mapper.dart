...
abstract class EnhancedDtoMapper<TEntity, TDto>
    extends ValidatedDtoMapper<TEntity, TDto> {
  ...
  Map<String, dynamic> _processIncomingData(Map<String, dynamic> data) {
    try {
      ...
      final migratedData =
          _schemaHandler.migrate(data, schemaVersion, currentSchemaVersion);

      // Apply nested transformations
      final transformedData =
          MappingUtils.transformNested(migratedData, nestedTransformers);
  ...
  Map<String, dynamic> _processOutgoingData(Map<String, dynamic> data) {
    ...
    final transformedData =
        MappingUtils.transformNested(data, _getReverseTransformers());
  ...
  ValidationResult _validateData(Map<String, dynamic> data,
      {required bool isDto}) {
    final errors = <String>[];
    ...
    try {
      ...
      final requiredValidation =
          MappingUtils.validateRequiredFields(data, requiredFields);
  ...
  List<MappingErrorReport> getRecentErrors({int limit = 10}) {
    return _errorReporter.getErrorsForMapper(runtimeType.toString(),
        limit: limit);
  }
