...
class MacroDtoMapper extends EnhancedDtoMapper<Macro, MacroDto>
  ...
  Map<String, dynamic> extractDataFromEntity(Macro entity) => {
        ...
        if (entity.aiInstruction != null)
          'ai_instruction': entity.aiInstruction,
        if (entity.lastUsed != null)
          'last_used': entity.lastUsed!.toIso8601String(),
  ...
  Macro createEntityFromData(Map<String, dynamic> data) {
    ...
    macro.category =
        MappingUtils.getNestedValue<String>(data, 'category') ?? 'General';
    macro.isFavorite =
        MappingUtils.getNestedValue<bool>(data, 'is_favorite') ?? false;
    macro.usageCount =
        MappingUtils.getNestedValue<int>(data, 'usage_count') ?? 0;
    macro.isAiMacro =
        MappingUtils.getNestedValue<bool>(data, 'is_ai_macro') ?? false;
    macro.aiInstruction =
        MappingUtils.getNestedValue<String>(data, 'ai_instruction');
