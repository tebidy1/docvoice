...
class AIStudioMultimodalService implements MultimodalAIService {
  ...
  Future<MultimodalAIResult> processAudioNote({
    ...
    try {
      ...
      String friendlyError = 'Gemini API Error: ${e.toString()}';
...
''';
      ...
      if (e.toString().toLowerCase().contains('429') || e.toString().toLowerCase().contains('quota')) {
         return MultimodalAIResult.error('⚠️ نفاذ الرصيد.', provider: _providerName);
      }
      return MultimodalAIResult.error('Gemini Error: ${e.toString()}', provider: _providerName);
    } catch (e) {
