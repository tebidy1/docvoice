...
import 'package:soutnote/services/desktop_injector.dart';
...
class InboxScreenState extends ConsumerState<InboxScreen> {
  ...
  Future<void> _copyAndMarkCopied(NoteModel note) async {
    final text = note.formattedText.isNotEmpty ? note.formattedText : note.content;

    // Desktop: use injector (Cmd+V/Win paste); Mobile/Web: copy only
    if (!Platform.isAndroid && !Platform.isIOS) {
      await DesktopInjector.instance.copyAndInject(text);
    } else {
  ...
  void _openArchive() async {
    final archivedNotes =
        await ref.read(inboxNoteRepositoryProvider).getArchived();
    if (!mounted) return;
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  Widget _buildNoteCard(BuildContext context, NoteModel note,
    ...
    if ((templateName == null || templateName.isEmpty) &&
      ...
      final macroIdStr =
          note.appliedMacroId?.toString() ?? note.suggestedMacroId?.toString();
      if (macroIdStr != null) {
