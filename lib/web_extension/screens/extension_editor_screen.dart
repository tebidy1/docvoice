...
import 'package:soutnote/core/services/groq_service.dart';
...
class _ExtensionEditorScreenState extends State<ExtensionEditorScreen> {
  ...
  // State
  List<Macro> _quickMacros = []; // Mobile service returns Macro
  ...
  Future<void> _loadMacros() async {
    ...
    // It returns List<Macro>
    final macros = await _macroService.fetchMacros();
  ...
  Future<void> _transcribeAudio(Uint8List bytes) async {
    try {
      ...
      if (!transcribed) {
        ...
        print(
            "DEBUG: Transcribing via Backend ($sttEngine)... Token present: ${apiService.hasToken}");
