...
class _AdminTemplatesScreenState extends State<AdminTemplatesScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to load templates: $e'), backgroundColor: Colors.red),
        );
  ...
  List<String> _getCategories(MacroModel macro) {
    ...
    return macro.category.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
  }
  ...
  Future<void> _toggleAssignment(MacroModel macro, bool assign) async {
    ...
    setState(() {
      ...
      if (assign) {
        ...
        cats.remove(_selectedDepartment!.nameEn); // remove legacy mapping if exists
      }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to save assignment: $e'), backgroundColor: Colors.red),
        );
        _loadData(); // revert
      }
    }
    ...
    showDialog(
      ...
      builder: (context) => _ManageAllMacrosDialog(
        ...
        onMacroDeleted: (macro) async {
          setState(() => _isLoading = true);
          Navigator.pop(context); // Close dialog briefly to reload, or update in place
          try {
            if (macro.id != null) {
        ...
        actions: [
          ...
          Padding(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('Templates Library', style: TextStyle(color: Colors.white, fontSize: 13)),
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  child: const Text('DEPARTMENTS', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold, fontSize: 11, letterSpacing: 1.5)),
                ),
                Expanded(
                  child: _isLoading
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final count = _allMacros.where((m) {
                              ...
                              return cats.contains(dept.id) || cats.contains(dept.nameEn);
                            }).length;

                            return ListTile(
                              dense: true,
                              leading: Icon(dept.icon,
                                  color: isSelected ? Colors.blue[300] : Colors.white38, size: 18),
                              ...
                              trailing: count > 0
                                  ? Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: isSelected ? Colors.blue.withOpacity(0.3) : Colors.white12,
                                        borderRadius: BorderRadius.circular(10),
                                      ),
                                      child: Text('$count',
                                          style: TextStyle(
                                            color: isSelected ? Colors.blue[300] : Colors.grey, fontSize: 11)),
                                    )
                                  : null,
                              selected: isSelected,
                ...
                ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text('Select a department to manage its templates',
                ...
                : _isLoading
                    ...
                    : Row(
                        children: [
                          ...
                          Expanded(
                            child: _buildMacroColumn(
                              title: 'ASSIGNED TO ${_selectedDepartment!.nameEn.toUpperCase()}',
      ...
      children: [
        Container(
          ...
          child: Row(
            ...
            children: [
              Text(title, style: TextStyle(color: isAssigned ? Colors.blue[300] : Colors.grey, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
              Text('${macros.length}', style: const TextStyle(color: Colors.grey)),
            ],
          ),
        ),
        Expanded(
          child: macros.isEmpty
              ? Center(child: Text(isAssigned ? 'No templates assigned' : 'No available templates', style: const TextStyle(color: Colors.white24)))
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      shape: RoundedRectangleBorder(
                        ...
                        side: BorderSide(color: isAssigned ? Colors.blue.withOpacity(0.3) : Colors.transparent, width: 1)
                      ),
                      ...
                      child: InkWell(
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              Icon(isAssigned ? Icons.remove_circle_outline : Icons.add_circle_outline, 
                                   color: isAssigned ? Colors.red[300] : Colors.green[300], 
                                   size: 20),
                              ...
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(macro.trigger, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
...
class _ManageAllMacrosDialogState extends State<_ManageAllMacrosDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final filtered = widget.macros.where((m) => 
    ...
    return AlertDialog(
      ...
      title: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          const Text('Global Templates Database', style: TextStyle(color: Colors.white)),
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          margin: const EdgeInsets.only(bottom: 8),
                          child: ListTile(
                            title: Text(macro.trigger, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                            subtitle: Text(
  ...
  void _confirmDelete(MacroModel macro) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete "${macro.trigger}"? This will remove it from all assigned departments.', style: const TextStyle(color: Colors.white70)),
        actions: [
...
class _TemplateEditDialogState extends State<_TemplateEditDialog> {
  ...
  void initState() {
    ...
    _contentController = TextEditingController(text: widget.macro?.content ?? '');
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (widget.macro == null) {
        await macroService.addMacro(macro); // Let service generate ID or hit API
      } else {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      backgroundColor: const Color(0xFF1E293B),
      title: Text(widget.macro == null ? 'Create New Template' : 'Edit Template', style: const TextStyle(color: Colors.white)),
      content: SizedBox(
        width: 600,
        child: Form(
          key: _formKey,
