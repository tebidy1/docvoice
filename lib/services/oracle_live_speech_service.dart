...
class OracleLiveSpeechService {
  ...
  Future<String> startSession(Stream<Uint8List> audioChunks) async {
    ...
    try {
      if (kIsWeb) {
        ...
        debugPrint('🔌 Calling _openWebSocket with inlineToken (length: ${webToken.length})');
  ...
  String _buildWsQueryParams({String? inlineToken}) {
    ...
    if (inlineToken != null && inlineToken.isNotEmpty) {
      ...
      debugPrint('🔑 WebSocket Auth: token (len=${inlineToken.length}) and compartmentId (len=${compartmentId.length}) added to URL');
      if (_backendCompartmentId != null) {
        debugPrint('   (Using compartmentId from backend)');
      }
    } else {
      debugPrint('⚠️ WebSocket Auth: inlineToken is null or empty! Token value: "$inlineToken"');
    }
  ...
  Future<void> _openWebSocket({String? inlineToken}) async {
    ...
    debugPrint('Query params string: ${queryParams.replaceAll(inlineToken ?? '', '<TOKEN>')}');
    ...
    debugPrint('WebSocket URL (from Uri): ${uriString.replaceAll(inlineToken ?? '', '<TOKEN>')}');
    
    // Verify token is actually in the URL
    if (inlineToken != null && !uriString.contains(inlineToken.substring(0, (inlineToken.length / 2).round()))) {
    ...
    try {
      ...
      debugPrint('   3. Oracle Speech service not enabled for this compartment');
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (jsonResponse['status'] == true && jsonResponse['token'] != null) {
            ...
            debugPrint('✅ Token extracted successfully (length: ${token.length})');
          ...
          if (jsonResponse['token'] != null) {
            ...
            final region = jsonResponse['region'] as String?;
            final compartmentId = jsonResponse['compartmentId'] as String?;
            debugPrint('✅ Token extracted (no status check, length: ${token.length})');
            return {
              'token': token,
              'region': region,
              'compartmentId': compartmentId,
          ...
          debugPrint('❌ Token not found in response. Keys: ${jsonResponse.keys.toList()}');
        }
        throw Exception(
  ...
  void _debugDecodeToken(String token) {
    try {
      ...
      if (parts.length != 3) {
        debugPrint('⚠️ Invalid JWT format (expected 3 parts, got ${parts.length})');
        return;
      }
      ...
      debugPrint('   iat: ${json['iat']} (${DateTime.fromMillisecondsSinceEpoch(json['iat'] * 1000)})');
      debugPrint('   exp: ${json['exp']} (${DateTime.fromMillisecondsSinceEpoch(json['exp'] * 1000)})');
      ...
      if (exp.isBefore(now)) {
        ...
        debugPrint('   ✅ Token is valid for ${exp.difference(now).inMinutes} more minutes');
      }
    } catch (e) {
