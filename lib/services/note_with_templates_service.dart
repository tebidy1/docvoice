...
import '../core/models/note_model_base.dart';
...
class NoteWithTemplatesService {
  ...
  Future<NoteModel> saveNoteWithTemplates({
    ...
    try {
      ...
      if (isNew) {
        ...
        savedNote = await _apiService.createNote(note as NoteModel);
        debugPrint('✅ Created new note with ID: ${savedNote.id}');
      } else {
        // Update note without outputs first
        savedNote = await _apiService.updateNote(note.id.toString(), note as NoteModel);
        debugPrint('✅ Updated note ID: ${savedNote.id}');
        
        // Step 2: Fetch existing outputs from server
        final existingNote = await _apiService.fetchNoteById(note.id.toString());
        ...
        savedNote = await _apiService.updateNote(
  ...
  Future<NoteModel> addTemplateToNote({
    ...
    final existingNote = await _apiService.fetchNoteById(noteId.toString());
    ...
    return await _apiService.updateNote(noteId.toString(), existingNote);
  }
  ...
  Future<GeneratedOutput?> getOutputForMacro({
    ...
    final note = await _apiService.fetchNoteById(noteId.toString());
    try {
