...
import 'package:soutnote/core/models/smart_suggestion.dart';
...
class _InboxNoteDetailViewState extends State<InboxNoteDetailView> {
  ...
  Future<void> _refreshNoteContent() async {
    try {
      ...
      if (freshNote != null) {
        final NoteModel note = freshNote;
        if (mounted) {
           setState(() {
             // 1. Update Text if empty (or force update if needed, but usually we trust local if user is typing)
             if (_generatedOutputs.isEmpty && note.generatedOutputs.isNotEmpty) {
                 _generatedOutputs = List.from(note.generatedOutputs);
                 _activeTabIndex = _generatedOutputs.length;
                 _finalNoteController.text = _generatedOutputs.last.content ?? "";
                 _isTemplateCardExpanded = false;
             } else if (_generatedOutputs.isEmpty && note.formattedText.isNotEmpty) {
                 final legacyName = note.summary ?? 'Legacy Note';
                 _generatedOutputs.add(GeneratedOutput(
                   title: legacyName, 
                   content: note.formattedText,
                   macroId: note.appliedMacroId,
                 ));
                 _activeTabIndex = 1;
                 _finalNoteController.text = note.formattedText;
                 _isTemplateCardExpanded = false;
             }
             
             // 2. Restore Selected Macro
             if (_selectedMacro == null && note.appliedMacroId != null) {
                 // Try to find in quick macros first
                 final found = _quickMacros.firstWhere(
                     (m) => m.id == note.appliedMacroId, 
                 ...
                 if (found.id != -1) {
                     ...
                     final appliedMacroId = note.appliedMacroId!;
                     _macroService.getAllMacros().then((all) {
                         final exact = all.where((m) => m.id == appliedMacroId).firstOrNull;
  ...
  Future<void> _loadQuickMacros() async {
    ...
    allowedMacros.sort((Macro a, Macro b) {
    ...
    if (mounted) {
      ...
      if (_selectedMacro == null && widget.note.appliedMacroId != null) {
          restoredMacro = allowedMacros.where((Macro m) => m.id == widget.note.appliedMacroId).firstOrNull;
