...
import 'package:soutnote/core/services/inbox_service.dart';
...
class _InboxNoteDetailViewState extends State<InboxNoteDetailView> {
  ...
  final _macroService = MacroApiService();
          ...
          setState(() {
            ...
            if (_selectedMacro == null && freshNote.appliedMacroId != null) {
              ...
              if (found.id != -1) {
                ...
                _macroService.fetchMacros().then((all) {
                  final exact = all
  ...
  Future<void> _loadQuickMacros() async {
    final favorites = await _macroService.fetchFavorites();
    var mostUsed = await _macroService.fetchMostUsed();

    // Fallback if most used is empty (e.g. fresh install)
    if (mostUsed.isEmpty && favorites.isEmpty) {
      final allMacros = await _macroService.fetchMacros();
  ...
  Future<void> _applyInsuranceTemplate() async {
    ...
    try {
      ...
      debugPrint("DetailView: Insurance generation error: $e");
  ...
  Widget _buildActionDock(ThemeData theme) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          if (_finalNoteController.text.isNotEmpty) ...[
            Align(
              ...
              child: ActionChip(
                ...
                onPressed: _isGenerating ? null : _applyInsuranceTemplate,
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(20),
